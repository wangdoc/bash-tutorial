...
`let`命令也只支持整数运算，变量计算中不需要加上 $ 来调用变量。
...
`bc`命令支持任意精度计算。默认是交互式的，在 linux 中可以当计算器使用。在脚本中通常使用管道调用。
...
默认情况下，`bc`命令不显示精度，可以通过`scale`指令指定要显示的小数位数。
```bash
# bc
...
`bc`命令支持比较运算。返回结果只有 0 和 1，如果结果为真则返回 1，为假则返回 0。
...
`bc`命令也支持逻辑运算。同样，返回结果只有 0 和 1，如果结果为真则返回 1，为假则返回 0。
```bash
# bc
