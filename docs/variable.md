...
如果变量不用双引号括起来，比如`echo $a`，那么隐含的意义就是，把`$a`变量的字符串，按照空格、制表符、换行符等符号来分割开，然后把这些分割后的每一项再按 `glob` 模式展开，最后把这些值，再用一个空格重新连接起来，并打印出来。 
 如果碰巧`$a`变量的字符串中，有连续两个空格符号，或者有制表符，那么`echo $a`就与程序员的思路出现了不一致。比如下例：
```bash
a="1 2    3   4"
$ echo $a
1 2 3 4
